...
class BaseSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        self.db = mysql.connect(host=VSENTI_DB_HOST, port=VSENTI_DB_PORT,
                user=VSENTI_DB_USER, passwd=VSENTI_DB_PASS, db=VSENTI_DB_NAME)
            ...
            try:
                ...
                if self.is_slack:
                    error_msg = '{}: Unable to update last_scraped_at: {}'.format(
                        spider.name, err)
                    self.slack.chat.post_message('#vsenti-scrapper-errors', error_msg,
                        as_user=True)
        else:
            if self.is_slack:
                # Send error to slack
                error_msg = '{}: Spider fail because: {}'.format(
                    spider.name, reason)
                self.slack.chat.post_message('#vsenti-scrapper-errors',
    ...
    def item_dropped(item, response, exception, spider):
        if self.is_slack:
            ...
            spider.slack.chat.post_message('#vsenti-scrapper-errors',
                    error_msg, as_user=True)
